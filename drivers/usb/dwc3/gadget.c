		...
		struct dwc3_gadget_ep_cmd_params *params)
{
	...
	dbg_log_string("UGMI: MEM BAR");
	/*
	 * some time to get ready before addressing all the registers to prevent SMMU fault.
	 */
	udelay(5);
	/*
	...
	dbg_log_string("UGMI: GCTL: %x", dwc3_readl(dwc->regs, DWC3_GCTL));
	dbg_log_string("UGMI: DCTL: %x", dwc3_readl(dwc->regs, DWC3_DCTL));
